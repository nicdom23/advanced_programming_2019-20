...
public:
	...
	std::string get() const; //use this instead of * operator (can work on const iterators)
	void next(); //use this instead of ++ operator
 	void previous();//use this instead of -- operator
	bool equals(Iterator b) const;//use this instead of == operator
private:
...
void Iterator::previous()//no needed in a foward iterator
{
...
Iterator pos;
for(pos = citizens.begin();!pos.equals(citizens.end());pos.next())
