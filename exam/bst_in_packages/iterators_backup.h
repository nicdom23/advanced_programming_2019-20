...
public:
	//custom constructor  	
	MyIterator(Node<T>* x,Node<T>* last_element)noexcept :pointer(x),last{last_element} {}
  	
	//copy constructor
	MyIterator(const MyIterator<T>& mit)noexcept : pointer(mit.pointer), last{mit.last} {}

	//copy assignment
	MyIterator& operator=(const MyIterator& x) noexcept{
		pointer = x.pointer;
		last = x.last;
		return *this;	
	}
