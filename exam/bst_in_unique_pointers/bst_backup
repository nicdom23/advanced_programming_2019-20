...
void transplant(Node<treepair>* a, std::unique_ptr<treepair> b) noexcept{
	...
	else if (a == (a->parent->left)){//a is not the root, a is the left child of his parent		
		Node<treepair>* temp1 = a->parent->left.release();
		a->parent->left.reset(b.release());//then b becomes the new left child of a's parent
	}
	else{
		Node<treepair>* temp2 = a->parent->right.release();
		a->parent->right.reset(b.release());//else b becomes the new right child of a's parent
			
	}
	if (b != nullptr){ 	
		std::cout<<"print declenched"<<std::endl;
